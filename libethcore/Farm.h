		...
		*/
		...
		public:
			...
			*/
			void setWork(WorkPackage const& _wp)
			{
				...
				for (auto const& m : m_miners)
				{
					...
					m->setWork(m_work);
					//this_thread::sleep_for(chrono::milliseconds(15));	//sleep some seconds; editor:wany
				}
				resetTimer();
			}
			...
			*/
			...
			{
				WorkingProgress p, p1;
				HANDLE handle;
				p1.ms = p.ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - m_lastStart).count();
				{
					ReadGuard l2(x_minerWork);
					for (auto const& i : m_miners)
					{
						p1.hashes = i->hashCount();
						p.hashes += p1.hashes;
						float mh = p1.rate() / 1000000.0f;
						char mhs[16];
						sprintf(mhs, "%.2f", mh);
						handle = GetStdHandle(STD_OUTPUT_HANDLE);
						SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY
							| FOREGROUND_GREEN | FOREGROUND_BLUE);
						printf("GPU#%d: %s MH/s;", i->index(), mhs);
					}
					cout << endl;
					SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY
						| FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
				}//change color and add every card info   editor:wany
